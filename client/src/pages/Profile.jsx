...
import { FiSave, FiLock, FiFileText, FiZap, FiBell, FiDatabase } from "react-icons/fi";
...
const AVAILABILITY_OPTIONS = ["Immediately", "2 Weeks Notice", "1 Month Notice", "Not Looking"];
...
export default function Profile() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <p className="text-slate-500 dark:text-slate-400 mb-8">Manage your personal information and account settings</p>
      ...
      {user?.role === "seeker" && (
        <div className="card p-6 sm:p-8 mt-6 space-y-3">
          <h2 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2"><FiDatabase size={16} /> Talent Database</h2>
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-900 dark:text-white">Make my profile discoverable to employers</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-md">
                When on, recruiters can find your headline, skills, experience level, and contact details by searching the talent database — even for jobs you haven't applied to. Your resume file is never shared this way; it's only sent when you actually apply. You can turn this off anytime.
              </p>
            </div>
            <button
              onClick={async () => {
                const next = !form.discoverable;
                setForm({ ...form, discoverable: next });
                try {
                  const { data } = await api.put("/users/me", { discoverable: next });
                  updateUserLocal(data.user);
                  toast.success(next ? "You're now visible in the talent database" : "You're hidden from the talent database");
                } catch (err) {
                  setForm({ ...form, discoverable: !next });
                  toast.error(getErrorMessage(err));
                }
              }}
              className={form.discoverable ? "btn-primary" : "btn-secondary"}
            >
              {form.discoverable ? "Visible" : "Hidden"}
            </button>
          </div>
        </div>
      )}
